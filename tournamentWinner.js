...
// TIME COMPLEXITY - O(n) -> Number of Teams
// SPACE COMPLEXITY - O(n) -> Number of Teams, each stored in hashMap + 2 for winnerScore and winnerTeam

function tournamentWinner(competitions, results) {
  let hashMap = {winnerScore: 0, winnerTeam: ''};

  competitions.forEach((competition, idx) => {
    let [homeTeam, awayTeam] = competition;
    let result = results[idx];
    let currWinner = result ? homeTeam : awayTeam;
    hashMap[homeTeam] = hashMap[homeTeam] || 0;
    hashMap[awayTeam] = hashMap[awayTeam] || 0;
    hashMap[currWinner] += 3;
    if (hashMap.winnerScore < hashMap[currWinner]) {
      hashMap.winnerScore = hashMap[currWinner];
      hashMap.winnerTeam = currWinner;
    }
  });
  return hashMap.winnerTeam;
}
